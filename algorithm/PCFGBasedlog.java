...
public class PCFGBasedlog {
	...
	public PCFGBasedlog(File trainFile, File dictionary) {
...
//			tmpcount = (int) (targetNumber*Double.parseDouble(guessResult.split("\t")[1]));
			tmpcount = (int) (targetNumber*Math.exp(Double.parseDouble(guessResult.split("\t")[1])));//to log
...
//								* replaceNode.getP();
						double newProb = probability - lastNode.getP() + replaceNode.getP();//to log
...
//						}
			...
			if (dmap[i] != null) {
				...
				for (int j = size - 1; j >= 0; j--) {
					digitbw.write(stri[j] + "\t"
							+ (Math.log(numb[j] * 1.0 / totalDigit[i])) + "\n");//to log
				}
				digitbw.close();
			}
			...
			if (smap[i] != null) {
				...
				for (int j = size - 1; j >= 0; j--) {
					symbolbw.write(stri[j] + "\t"
							+ (Math.log(numb[j] * 1.0 / totalSymbol[i])) + "\n");//to log
				}
				symbolbw.close();
			}
			if (lmap[i] != null) {
				...
				for (int j = size - 1; j >= 0; j--) {
					lowerbw.write(stri[j] + "\t"
							+ (Math.log(numb[j] * 1.0 / totalLower[i])) + "\n");//to log
				}
				lowerbw.close();
			}
			if (umap[i] != null) {
				...
				for (int j = size - 1; j >= 0; j--) {
					upperbw.write(stri[j] + "\t"
							+ (Math.log(numb[j] * 1.0 / totalUpper[i])) + "\n");//to log
				}
				upperbw.close();
			}
		}
	...
	public void genGrammar(String trainingSet) throws Exception {
		...
		for (int i = size - 1; i >= 0; i--) { // the reverse order because we
			...
			bw.write(str[i] + "\t" + (Math.log(num[i] * 1.0 / count)));//to log
...
//		int[] rounds = {3,6,10};
		int round=2;
		String[] trainingSets = {dataSet+"_1sixth",dataSet+"_1third",dataSet+"_1tenth"};
//		for(int i=0;i<trainingSets.length;i++){
		for(int i=0;i<1;i++){
//			round = rounds[i];
			for(int r=1;r<=round;r++){
				PCFGBasedlog pcfgBased = new PCFGBasedlog();
				pcfgBased.targetNumber = targetNumber;
